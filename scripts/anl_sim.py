...
if __name__ == "__main__":
    ...
    while not rospy.is_shutdown():
        ...
        try:
            print(f"{bcolors.HEADER}Sim_T: {anl_sim.sim_t:.2f}s | {bcolors.OKCYAN}Dhw: {anl_sim.front_s - anl_sim.ego_s:.2f}m | {bcolors.OKCYAN}d_V: {anl_sim.front_v - anl_sim.ego_v:.2f}m/s | {bcolors.OKCYAN}d_A: {anl_sim.front_a - anl_sim.acc:.2f}m/s² | {bcolors.OKGREEN}Ego_S: {anl_sim.ego_s:.2f}m | Ego_V: {anl_sim.ego_v*2.237:.2f}mph  | Ego_A: {anl_sim.acc:.2f}m/s²{bcolors.ENDC}", end="\r")
